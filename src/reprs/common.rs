...
pub trait ImportResolver: Copy {
    fn resolve(self, current: ImportId, path: &str) -> Result<ImportId, String>;
}
impl<T: Copy + FnOnce(ImportId, &str) -> Result<ImportId, String>> ImportResolver for T {
    fn resolve(self, current: ImportId, path: &str) -> Result<ImportId, String> {
        self(current, path)
    }
}

pub trait ImportReader<'i> {
    fn read(&self, import: ImportId) -> Result<&'i FileInfo<'i>, String>;
}
impl<'i, T: Copy + FnOnce(ImportId) -> Result<&'i FileInfo<'i>, String>> ImportReader<'i> for T {
    fn read(&self, import: ImportId) -> Result<&'i FileInfo<'i>, String> {
        self(import)
    }
}
