...
mod context {
    ...
    pub(super) struct ContextInner<'i, IR: ImportResolver> {
        import_resolver: IR,
        import_id: ImportId,
        imports_resolved: RefCell<HashMap<ImportId, (&'i str, Span<'i>)>>,
    }

    impl<'i, IR: ImportResolver> ContextInner<'i, IR> {
        pub(super) fn new(import_id: ImportId, import_resolver: IR) -> Self {
            Self {
                import_resolver,
                import_id,
    ...
    impl<'i, 'inn, IR: ImportResolver> Context<'i, 'inn, IR> {
        pub(super) fn new(inner: &'inn ContextInner<'i, IR>) -> Self {
            Self {
                inner,
...
pub fn validate<'i>(
    ...
    let inner = ContextInner::new(import_id, import_resolver);
