...
mod context {
    ...
    impl<'a, 'inn> Context<'a, 'inn> {
        ...
        pub(super) fn get_var_ty(&self, index: Idx) -> Option<InternedType<'a>> {
            ...
            Some(var_ty.deepen(ty_eff_lvl, cur_ty_eff_lvl, self))
        }
        ...
        let (term, ty, effects) = match term {
            ...
            uir::RawTerm::TyAbs { name, bounds, body } => {
                ...
                let (check_result, infer) = if let Some(check_type) = take_concrete_check_type()? {
                    ...
                    {
                        if *check_name != CONCRETE_TY_APP_NAME {
                            // if check_name == CONCRETE_TY_APP_NAME, the bounds will be
                            // checked afterwards so its fine to leave it until then
            ...
            uir::RawTerm::EffAbs { name, body } => {
                let (check_result, infer) = if let Some(check_type) = take_concrete_check_type()? {
                    ...
                    } = check_type
                    {
                        ...
                        // expected something other than an effect abstraction and we are allowed to
                        // infer eff_args so we attempt to do so
                        // we cannot pass down check_type as the eff_var contexts would diverge
                        (None, Some(check_type))
                    } else {
                        Err(SpannedError::ty_mismatch(
                            check_type.display(ctx)?,
                            "found an effect abstraction (could not infer)",
                            *info,
                        ))?
                    }
                } else {
                ...
                let ctx_ = ctx.push_eff_var(name.0, EffVar::Unbound);
                let (body, result, effects_used) =
                    body.type_check(check_result, ty_config, &ctx_)?;

                if !effects_used.is_empty() {
                    Err(SpannedError::with_context(
                        "effect abstraction cannot capture effects",
                        "consider adding an intermediate abstraction",
                        "effect abstraction here",
                        *info,
                        effects_used
                            .spans_iter()
                            .map(|span| (*span, "effect used here")),
                ...
                let eff_abs = ctx.intern(Type::EffAbs {
                    name: *name,
                    result,
                });
            ...
            uir::RawTerm::Trigger(effect_term) => {
                ...
                let (term, ty) = match effect.concrete(ctx.next_ty_eff_level(), ctx)? {
                    ...
                    Effect::Var(level, eff_kind) => match ctx.get_eff_var_unwrap(level)?.1 {
                        ...
                        EffVar::Effect { .. } => Err(IllegalError::new(
                            format!(
