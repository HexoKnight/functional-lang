...
use crate::{
    ...
    reprs::{
        ast,
        common::{FileInfo, ImportId, ImportReader, ImportResolver, Span},
...
impl Pipeline {
    ...
    pub fn validate_rec<'i>(
        ...
        import_reader: impl ImportReader<'i>,
    ) -> Result<Vec<(ImportId, untyped_ir::Term<'i>)>, CompilationError<'i>> {
        let mut to_validate = VecDeque::new();
        let mut validated = HashMap::new();
        let mut dependency_graph = HashMap::new();

        let mut handle_source = |import_id, file_info, to_validate: &mut VecDeque<_>| {
            let ast = self.parse_single(file_info)?;

            let (untyped_ir, dependencies) = validate(import_id, &ast, import_resolver)?;
        ...
        handle_source(initial, initial_file_info, &mut to_validate)?;
        ...
        while let Some((import_id, (path, span))) = to_validate.pop_front() {
            ...
            handle_source(import_id, source, &mut to_validate)?;
        }
    ...
    pub fn type_check_rec<'i>(
        ...
        let untyped_irs =
            self.validate_rec(initial, initial_file_info, import_resolver, import_reader)?;
    ...
    pub fn evaluate_rec<'i>(
        ...
        let mut imports =
            self.type_check_rec(initial, initial_file_info, import_resolver, import_reader)?;
